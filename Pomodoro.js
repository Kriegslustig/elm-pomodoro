...
Elm.PomodoroBackground.make = function (_elm) {
   "use strict";
   _elm.PomodoroBackground = _elm.PomodoroBackground || {};
   if (_elm.PomodoroBackground.values) return _elm.PomodoroBackground.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var calcHeight = F2(function (state,max) {    var p = state / max * 100;return A2($Basics._op["++"],$Basics.toString(p),"%");});
   ...
   var update = F2(function (action,model) {
      ...
      {case "Nope": return model;
         case "Increment": return _U.update(model,{state: model.state + 1.0});
         default: return _U.update(model,{state: model.state - 1.0});}
   });
...
Elm.PomodoroBell.make = function (_elm) {
   "use strict";
   _elm.PomodoroBell = _elm.PomodoroBell || {};
   if (_elm.PomodoroBell.values) return _elm.PomodoroBell.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var view = F2(function (address,model) {    return A2($Html.audio,_U.list([$Html$Attributes.src(model.url)]),_U.list([]));});
   var init = function (url) {    return {state: 0,url: url};};
   var update = F2(function (action,model) {
      var _p0 = action;
      if (_p0.ctor === "NoOp") {
            return model;
         } else {
            return _U.update(model,{state: model.state + 1});
         }
   });
   var StartAudio = {ctor: "StartAudio"};
   var NoOp = {ctor: "NoOp"};
   var Model = F2(function (a,b) {    return {state: a,url: b};});
   return _elm.PomodoroBell.values = {_op: _op,update: update,view: view,init: init,Model: Model,NoOp: NoOp,StartAudio: StartAudio};
};
...
Elm.Pomodoro.make = function (_elm) {
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $PomodoroBackground = Elm.PomodoroBackground.make(_elm),
   $PomodoroBell = Elm.PomodoroBell.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   ...
   var init = {time: 0
              ...
              ,running: true
              ,length: 0
              ,background: $PomodoroBackground.init(0)
              ,bell: $PomodoroBell.init("./static/bell.ogg")};
   var getCycle = F2(function (cycle,round) {
      return A2($Maybe.withDefault,0,$List.head($List.reverse(A2($List.take,A2($Basics._op["%"],round + 1,$List.length(cycle)),cycle))));
   });
   var runningToggleValue = function (running) {    return running ? "Pause" : "Start";};
   var cycle = _U.list([25 * 60,5 * 60,25 * 60,5 * 60,25 * 60,5 * 60,25 * 60,10 * 60]);
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "Nope": return model;
         ...
         case "UpdateBg": return _U.update(model,{background: A2($PomodoroBackground.update,_p0._0,model.background)});
         default: return _U.update(model,{bell: A2($PomodoroBell.update,_p0._0,model.bell)});}
   });
   var UpdateBell = function (a) {    return {ctor: "UpdateBell",_0: a};};
   var UpdateBg = function (a) {    return {ctor: "UpdateBg",_0: a};};
   var Reset = {ctor: "Reset"};
   var ToggleRunning = {ctor: "ToggleRunning"};
   var NewRound = {ctor: "NewRound"};
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
   ...
   var Nope = {ctor: "Nope"};
   var actions = $Signal.mailbox(Nope);
   var model = A3($Signal.foldp,update,init,A2($Signal.merge,actions.signal,incTime));
   var playSound = Elm.Native.Port.make(_elm).outboundSignal("playSound",
   function (v) {
      return v;
   },
   A2($Signal.map,function (model) {    return model.bell.state;},model));
   var main = A2($Signal.map,view(actions.address),model);
   var Model = F8(function (a,b,c,d,e,f,g,h) {    return {time: a,round: b,work: c,pause: d,running: e,length: f,background: g,bell: h};});
   return _elm.Pomodoro.values = {_op: _op
                                 ...
                                 ,cycle: cycle
                                 ,runningToggleValue: runningToggleValue
                                 ,incTime: incTime
                                 ,getCycle: getCycle
                                 ,update: update
                                 ,view: view
